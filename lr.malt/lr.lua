-- This is a linear regression example from Torch tutorial repo
-- This has been modified to support malt-2
...
require 'torch'
--require 'optim' -- malt2 requires removing optim and using dstoptim 
-- (and dstsgd instead of sgd as an optimizer). 
require 'nn'
require 'dstoptim'

torch.setdefaulttensortype("torch.FloatTensor") -- current malt2 support is limited to floattensors 

-- We will write the loss to a text file and read from there to plot the loss as training proceeds
logger = optim.Logger('loss_log_'..optim.iproc()..'.txt')
...
--sgd_params.init = false
--sgd_params.transport = 'mpi'
--sgd_params.cuda = false
...
for e = 1,1e4 do
   ...
   for i = 1,(#data)[1] do
      ...
      _,fs = optim.dstsgd(feval,x,sgd_params) -- change sgd to dstsgd only change required!!
